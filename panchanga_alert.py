...
# Janma nakshatras (1-27), comma-separated. Default: 22,17 = Shravana, Anuradha.
# Chandrashtama = the 8th nakshatra (inclusive) from each janma nakshatra.
JANMA_NAKSHATRAS = [
    int(x.strip()) for x in os.environ.get("JANMA_NAKSHATRA", "22,17").split(",") if x.strip()
]
...
def main() -> None:
    ...
    # Chandrashtama: 8th nakshatra (inclusive) from each janma, wraps mod 27.
    for janma in JANMA_NAKSHATRAS:
        chandrashtama_nak = ((janma - 1 + 7) % 27) + 1
        if nak_num == chandrashtama_nak:
            janma_name = NAKSHATRAS[janma - 1]
